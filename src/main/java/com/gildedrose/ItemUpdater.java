...
class ItemUpdater {
	...
	private static int outdatedDegradationFactor(Item item) {
		return (item.sellIn < SELL_IN_LIMIT) ? 2 : 1;
	}

	private static void increaseQuality(Item item, int factor) {
		item.quality = Math.min(item.quality + (factor * QUALITY_STEP), QUALITY_MAX);
	}

	private static void decreaseQuality(Item item, int factor) {
		item.quality = Math.max(item.quality - (factor * QUALITY_STEP), QUALITY_MIN);
	}

	private static int qualityFactor(Item item) {
